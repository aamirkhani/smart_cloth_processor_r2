   ...
   public class HIDUSBDevice: IDisposable
    {
        ...
        //by Papiya
        //connect to two devices each having the same vendorID and productID but different string in the complete HID string
        ...
        //by Papiya
        //searches for devices having the same vendorID and productID
        ...
        /// <returns></returns>
        public bool writeData(string devicePath, byte[] bDataToWrite)
        {
            bool success = false;
                   ...
                   // int code = myUSB.CT_HidP_GetCaps(myPtrToPreparsedData);
                    ...
                    while (bytesSend < bDataToWrite.Length)
                    {
                       ...
                       // byte[] OutputReportBuffer = new byte[myUSB.myHIDP_CAPS.OutputReportByteLength - 1 + 1];
                        ...
                        //Papiya -- getting Hid handle if we have more than one devices with the same VendorId and ProductID
                        int hidHandle = myUSB.getHidHandleForDevicePath(devicePath);
                        OutputReport myOutputReport = new OutputReport();
                        success = myOutputReport.Write(OutputReportBuffer, hidHandle);
                    }
                }
