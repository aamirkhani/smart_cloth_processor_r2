...
namespace USBHIDDRIVER.USB
{
    ...
    {
        //Papiya - added this to hold information when there are more than one devices with the same vendorId and ProductId
        Dictionary<string, int> dictDevPathHid = new Dictionary<string, int>(); 
        ...
        public unsafe int CT_CreateFile(string DeviceName)
		{
			...
			{
                //Papiya added to store info when there are more than one devices with the same vendorId and ProductId
                dictDevPathHid.Remove(DeviceName);
                dictDevPathHid.Add(DeviceName, HidHandle);
                return 1;
			}
        ...
        //Papiya added to store info when there are more than one devices with the same vendorId and ProductId
        public int getHidHandleForDevicePath(string devPath)
        {
            ...
            {
                foreach (KeyValuePair<string, int> item in dictDevPathHid)
                {
                    if(item.Key.Equals(devPath))
                    {
                        return item.Value;
                    }
